...
export default function Home() {
  ...
  const chooseWinner = (winner) => {
    ...
    const chosenPrompt = winner === "A" ? abData.promptA : abData.promptB;

    const chosenText = winner === "A" ? abData.resultA : abData.resultB;
  ...
  const advancedText = `Power Mode Unlocked
Accuracy • Smarter • Dynamic`;
  ...
  useKeyboardShortcuts({
    ...
    search: () => window.dispatchEvent(new Event("openSearch")),

    dashboard: () => navigate("/dashboard"),

    templates: () => window.dispatchEvent(new Event("openTemplates")),

    promptPanel: () => {
      if (selectedType) setPromptPanelOpen((prev) => !prev);
    },
...
`}
      >
        ...
        <TopBar activeChatId={activeChatId} selectedType={selectedType} />
        <TemplatesBar
                        ...
                        <>
                          ...
                          {!isUser && (
                            <div className="flex gap-4 text-xs opacity-70 mt-3">
                              ...
                              <span>
                                Readability: {getReadabilityScore(msg.content)}
                                /100
                              </span>
                            </div>
                          )}
                  ...
                  <button
                    onClick={async () => {
                      ...
                      for (let i = 0; i < batchInputs.length; i++) {
                        ...
                        setBatchProgress(
                          Math.round(((i + 1) / batchInputs.length) * 100),
                        );
                      }
...
${templatesOpen ? "right-[380px]" : "right-0"}
  `}
          ...
          <div className="max-w-4xl mx-auto px-6 pb-6 space-y-4">
            ...
            <div className="bg-gray-200 dark:bg-black/80 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl">
              <PromptBox
                ...
                onResult={(aiText, returnedChatId) => {
                  // ⭐ server created new chat session
                    ...
                    for (let i = updated.length - 1; i >= 0; i--) {
                      if (
                        ...
                        updated[i] = {
                          role: "assistant",
                          content: aiText,
                          id: Date.now(), // ⭐ required for editor
                          createdAt: Date.now(),
                        };
                        break;
                      }
      ...
      <Toast show={showSuccessToast} message="Content generated successfully" />
    </div>
  );
}
