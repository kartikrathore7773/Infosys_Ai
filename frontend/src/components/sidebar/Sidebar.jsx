...
export default function Sidebar({ isOpen, onSelectChat }) {
  ...
  const filteredHistory = useMemo(() => {
    return history.filter((chat) => {
      ...
      if (dateFilter === "7 Days") dateMatch = (now - created) / 86400000 <= 7;
  ...
  const saveRename = (chatId) => {
    ...
    const updated = history.map((chat) =>
      chat.chatId === chatId ? { ...chat, title: renameValue.trim() } : chat,
    );
  ...
  ${isOpen ? "w-72" : "w-20"}`}
      ...
      `}
            ...
            <div
              className={`mt-6 space-y-2 ${!isOpen && "flex flex-col items-center"}`}
            >
              <SidebarItem
                icon={Search}
                label={
